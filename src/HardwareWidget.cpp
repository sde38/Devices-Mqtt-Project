...
HardwareWidget::HardwareWidget(const HardwareDevice& device, QWidget* parent) 
    : QWidget(parent), m_deviceId(device.id) 
{
    // Design basique de la carte (Style CSS optionnel pour faire joli)
    setAttribute(Qt::WA_StyledBackground, true);
    setStyleSheet("HardwareWidget { background-color: #2d2d2d; color: white; border-radius: 8px; padding: 10px; }");

    auto* layout = new QVBoxLayout(this);
    
    m_nameLabel = new QLabel(QString::fromStdString(device.name + " (" + device.type + ")"), this);
...
void HardwareWidget::updateData(float temp, float load) {
